...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const userId = user.id; // always exists from backend
    if (!userId) {
    ...
    fetch(`${API_URL}/subcategories?userId=${userId}`)
      .then((res) => res.json())
      .then((data) => setSubcategoryCount(data.length));

    fetch(`${API_URL}/product?userId=${userId}`, {
    ...
    <div className="d-flex">
      <div className="flex-grow-1 p-3 rounded-3" style={{ backgroundColor: "#331159" }}>
        ...
        <div className="bg-primary text-white p-3 d-flex justify-content-between align-items-center mt-3">
          <h6 className="mb-0">
            {user?.memberId ? "User Dashboard" : "Admin Dashboard"}
          </h6>
